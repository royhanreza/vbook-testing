        ...
        <div class="container-fluid">
            <div class="row" style="height: 100vh;">
                ...
                <div class="col-md-5 p-0 py-3 ">
                    ...
                    <span class="badge badge-secondary m-0 font-weight-normal text-white">IP Address - {{ $ip_address->ip_address }}</span>
                    <span class="badge badge-secondary m-0 font-weight-normal text-white">@{{ bookingOngoing[0]?.title }}</span>
                ...
                <div class="col-md-4 px-4 py-3">
                        ...
                        <div class="text-center">
                            <div class="card" v-if="bookingOngoing.length > 0">
                                <div class="card-body">
                                    ...
                                    @if ($bookingOngoing->count() > 0)
                                    {!! QrCode::size(120)->backgroundColor(255,90,0)->generate($linkQr) !!}
                            ...
                            <h5> <span class="badge badge-success text-white mb4" v-if="bookingOngoing.length > 0">@{{ bookingOngoing[0]?.title}}</span></h5>

                            <h5> <span class="badge badge-danger text-white py-2 mb-4" v-if="bookingOngoing.length < 1">@{{ bookingNull }}</span></h5>
                    ...
                    <div class="style-after-upcoming">
                        ...
                        <div style="overflow: auto; height: 37vh;">
                            ...
                            <p id="rcorners2" class="text-white h6" v-if="bookingTodays.length > 0"> @{{ bookingTodays[0]?.title }} - @{{ bookingTodays[0]?.start_date }}</p>

                            <p id="rcorners3" class="text-white h6" v-else>@{{ bookingTodaysNull }}</p>
        ...
        const vue = new Vue({
            ...
            methods: {
                bookingOnGoing: function() {
                    let vm = this;
                    var VBOOKIP = 'http://' + '{!! $ip_address->ip_address !!}';
                ...
                bookingToday: function() {
                    let vm = this;
                    axios.get('{{ route("room.api-display") }}').then(res => {
                        vm.bookingTodays = res.data.booking_today;
